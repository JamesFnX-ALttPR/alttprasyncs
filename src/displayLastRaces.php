...
while ($row = $stmt->fetch()) {
    ...
    echo $startOfRow . '<td>' . $raceStart . '</td><td>' . $raceMode . '</td><td>' . $raceDescription . '</td><td><a target="_blank" href="https://racetime.gg/alttpr/' . $raceSlug . '">' . $raceSlug . '</a></td><td><a target="_blank" href="' . $raceSeed . '">Download Seed</a></td><td>' . hashToImages($raceHash) . '</td><td>' . $participantCount . '<td><a href="' . $domain . '/async/' . $raceID . '">Submit Async</a></td><td><a href="' . $domain . '/results/' . $raceID . '">View Results</a></td></tr>' . PHP_EOL;
}
echo '            </tbody>' . PHP_EOL;
echo '        </table>' . PHP_EOL;
