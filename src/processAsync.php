...
//Variables passed from form: seed (required), mode (required). hash1-5 (required), description, spoiler, spoiler_log, team, login_required, vod_required, edits_allowed, tournament_seed
...
$hash = '(' . $_POST['hash1'] . '/' . $_POST['hash2'] . '/' . $_POST['hash3'] . '/' . $_POST['hash4'] . '/' . $_POST['hash5'] . ')';
...
if ($errors != null) {
    ...
    echo '                <tr><th class="rightAlign">Hash:</th><td>' . hashToImages($hash) . '</td></tr>' . PHP_EOL;
    if ($description != '') {
