...
if ($_POST['hash1'] != '' && $_POST['hash2'] != '' && $_POST['hash3'] != '' && $_POST['hash4'] != '' && $_POST['hash5'] != '') {
    $searchHash = '(' . $_POST['hash1'] . '/' . $_POST['hash2'] . '/' . $_POST['hash3'] . '/' . $_POST['hash4'] . '/' . $_POST['hash5'] . ')';
} else {
    $searchHash = '';
}
...
if ($searchTerm == '' && $searchHash == '') {
    ...
    if (! $searchCheck) {
        ...
        echo '<input type="hidden" id="hash1" name="hash1" value="' . $_POST['hash1'] . '" /><input type="hidden" id="hash2" name="hash2" value="' . $_POST['hash2'] . '" /><input type="hidden" id="hash3" name="hash3" value="' . $_POST['hash3'] . '" /><input type="hidden" id="hash4" name="hash4" value="' . $_POST['hash4'] . '" /><input type="hidden" id="hash5" name="hash5" value="' . $_POST['hash5'] . '" />' . PHP_EOL;
        ...
        if ($searchTerm != '' && $searchHash != '') {
            echo $searchTerm . ' - ' . hashToImages($searchHash);
        } elseif ($searchTerm != '' && $searchHash == '') {
            echo $searchTerm;
        } else {
            echo hashToImages($searchHash);
        }
        ...
        while($row = $stmt->fetch()) {
            ...
            echo '</td><td><a target="_blank" href="' . $raceSeed . '">Download Seed</a></td><td>' . hashToImages($raceHash) . '</td><td>' . $participantCount . '<td><a href="' . $domain . '/async/' . $raceID . '">Submit Async</a></td><td><a href="' . $domain . '/results/' . $raceID . '">View Results</a></td>';
            if ($seriesColumn == 'y') {
